...
Image transform::transform(const Image& image, const Image& deformation)
{
    if (image.valid() && deformation.valid())
    {
        ...
        //    PYTHON_ERROR(PyExc_ValueError, "Deformation field and image needs to be the same size");
        ...
        {
            PYTHON_ERROR(PyExc_ValueError, "Unsupported image format (%s)", image::pixel_type_to_string(image_to_transform.pixel_type()));
        }
    ...
    PYTHON_ERROR(PyExc_ValueError, "Missing arguments");
}
