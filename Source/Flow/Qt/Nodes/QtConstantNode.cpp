...
void QtConstantNode::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget)
{
    Q_UNUSED(option);
    Q_UNUSED(widget);
...
void QtConstantNode::create_pins()
{
...
void QtConstantNode::calculate_size()
{
    assert(_node->pins().size() == 1);
...
const QString& QtConstantNode::value() const
{
    return _value;
}
